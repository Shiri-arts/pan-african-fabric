...
The September 13 source-truth pass added only unambiguous draft content. Semantic identity checks now detect a matching slug, region key/name or designer display name even when Wix uses a different item ID. Existing matching records are skipped as no-ops, and published records are never overwritten. The pass created five Regions and nine minimal Designers, and filled Edition One's `colourNarrative` and `creativeProcess` fields. It did not create Symbols, Participations, Garments, additional Events, MediaAssets, Stories, PressItems, PartnershipOptions, ContactChannels or ShopItems because their source mappings still require review, classification, complete relationships or rights approval.
