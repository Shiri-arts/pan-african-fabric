...
The 18-collection redesign and additive access readback succeeded on site `6dabfd00-04c6-4f6f-8282-fb56b240c160` on 2026-09-12. Five foundation records remain published: two Pages, one PageSection and two MediaAssets. Forty-four migrated records remain draft-only: one SiteSettings record, six Pages, one Edition, five Regions, nine Countries, twelve EditionColours, nine Designers and one Event. Anonymous access to every published base collection succeeds and anonymous draft-collection requests are denied. `../../cms-access-verification.json` records the latest completed remote verification.

The September 13 source-truth pass added only unambiguous draft content. Semantic identity checks now detect a matching slug, region key/name, designer display name, media filename or media URL even when Wix uses a different item ID. Existing matching records are skipped as no-ops, and published records are never overwritten. The pass created five Regions and nine minimal Designers, filled Edition One's `colourNarrative` and `creativeProcess` fields, and created ten MediaAssets drafts for the exact uploaded source files. Those media records remain `review-only`, cannot be downloaded, and omit unresolved creator, copyright, credit and approval metadata. It did not create Symbols, Participations, Garments, additional Events, Stories, PressItems, PartnershipOptions, ContactChannels or ShopItems because their source mappings still require review, classification, complete relationships or rights approval.
