...
export interface CmsContactChannel extends CmsEntity {
  ...
  readonly phoneNumber?: string; readonly responseNote?: string;
}
...
export function createCmsContentAccess(source: PublicCmsSource, scaleWixImage: (uri: string, width: number, height: number) => string, options: { allowReviewMedia?: boolean } = {}) {
  ...
  return Object.freeze({
    ...
    async getEvents(): Promise<readonly CmsEvent[]> { return mapList('Events', undefined, mapEvent, [{ field: 'startsAt', direction: 'desc' }, { field: '_id', direction: 'asc' }]); },
  ...
  function mapContact(item: CmsItem): CmsContactChannel | undefined { const channelKey = text(item.channelKey, 100); const label = text(item.label, 200); const email = text(item.emailAddress, 320); const phone = text(item.phoneNumber, 50); if (!channelKey || !label || !bool(item.publicUseVerified) || item.isEnabled === false) return undefined; return withBase(item, { channelKey, label, description: text(item.description, 1000), emailAddress: email && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email) ? email : undefined, phoneNumber: phone && /^\+?[0-9 ()-]{7,30}$/.test(phone) ? phone : undefined, responseNote: text(item.responseNote, 1000) }); }
